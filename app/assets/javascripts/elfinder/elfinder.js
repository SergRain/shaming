	...
	if (opts == 'open') {
		...
		if (node.is(':hidden')) {
			...
			$('body').find(':visible').each(function() {
				...
				if (this !== node[0] && $this.css('position') == 'absolute' && (z = parseInt($this.css('z-index'))) > zindex) {
					zindex = z + 1;
				}
			});
                        node.css({'z-index': zindex}).css(pos).show().trigger('resize')
	...
	return this.each(function() {
		...
		if ($.isArray(cmd.variants)) {
			...
			menu = $('<div class="ui-widget ui-widget-content elfinder-button-menu ui-corner-all"/>')
				.hide()
				.appendTo(button)
                                .css({'z-index': 12+button.css('z-index')})
			...
			open = function(x, y) {
				var win        = $(window),
					...
					css        = {
						...
						'z-index' : 100 + fm.getUI('workzone').css('z-index')
					};
			...
			dialog = $('<div class="ui-dialog ui-widget ui-widget-content ui-corner-all ui-draggable std42-dialog  '+cldialog+' '+opts.cssClass+'"/>')
				...
				.mousedown(function(e) {
					...
					if (!dialog.is('.'+clactive)) {
						...
						dialog.addClass(clactive).css({'z-index': maxZIndex() + 1});
					}
				})
				.bind('open', function() {
					dialog.trigger('totop');
				...
				.bind('totop', function() {
					...
					overlay.css({'z-index': $(this).css('z-index')});
				})
				.data({modal: opts.modal}),
				maxZIndex = function() {
					var z = parent.css('z-index') + 10;
	...
	this.not('.elfinder-navbar').each(function() {
		...
		if ($.fn.resizable) {
			handle = nav.resizable({
				...
				.find('.ui-resizable-handle').css({'z-index': nav.css('z-index') + 10});
...
$.fn.elfinderoverlay = function(opts) {
	...
	if (opts == 'show') {
		...
		if (o.is(':hidden')) {
                        o.css({'z-index': o.parent().css('z-index')+1});
...
elFinder.prototype.commands.quicklook = function() {
	...
	this.init = function() {
		...
		fm.one('load', function() {
			parent = fm.getUI();
			cwd    = fm.getUI('cwd');
                        win.appendTo('body').css({'z-index': 100+parent.css('z-index')});
		...
		var fm    = this.fm,
			...
			open = function(file, id) {
				...
				fm.dialog(dialog, {
					...
					destroyOnClose : true,
					buttons        : buttons,
                                        open           : function() { preview.css({'z-index': 1+$(this).parent().css('z-index')});}
